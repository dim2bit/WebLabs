...
function get_birthdate(person, consonants, vowels) {
  ...
  birthdate_array[0] = parseInt(birthdate_array[0]);
  if (person.gender == "M")
    if (birthdate_array[0] < 10) 
      fc_day = "0" + birthdate_array[0];
    else 
      fc_day = birthdate_array[0];
  else
    fc_day = birthdate_array[0] + 40;
...
function fiscalCode(person) {
  ...
  let fc_surname = get_surname(person, consonants, vowels);
  let fc_name = get_name(person, consonants, vowels);
  let fc_birthdate = get_birthdate(person, consonants, vowels);
  return fc_surname+fc_name+fc_birthdate;
}

let person = new Person("Dima", "Lobunko", "M", "1/1/1900");
